package com.mt.newsdemo.weather;
...
public class WeatherJsonUtils {
    ...
    public static String getCity(String json) {
        ...
        if (element != null && "OK".equals(element.getAsString())) {
            JsonElement resultElemt = object.get("result");
            JsonObject resultObj = resultElemt.getAsJsonObject();
            JsonElement addressCom = resultObj.get("addressComponent");
            JsonObject addressObj = null;
            if (addressCom != null) {
                addressObj = addressCom.getAsJsonObject();
                if (addressObj != null) {
                    JsonElement cityElement = addressObj.get("city");
                    if (cityElement != null) {
                        String str = cityElement.getAsString();
                        String city = str.replace("市", "");
                        return city;
                    }
    ...
    public static List<WeatherBean> getWeatherInfo(String json){
        ...
        if("1000".equals(status)){
            JsonArray jsonArray = jsonObject.get("data").getAsJsonObject().get("forecast").getAsJsonArray();
            if(jsonArray == null){
                return null;
            }
            for(int i = 0 ; i < jsonArray.size();i++){
                WeatherBean bean = getWeatherBeanFromJson(jsonArray.get(i).getAsJsonObject());
                list.add(bean);
            }
        }
